...
- **节流**:同类音效 200ms 时间窗内去重(初值,可在设置中调整);适用于 button/confirm 等高频槽位。两类例外:①菜单移动音在交互场景(悬停/键盘移动)是合法高频连响,不套 200ms 去重,改用「50ms 最小间隔 + 单声道打断」(新响立即停旧响,不叠加);②交互音事件(§5.1 开/选项点击/取消)按事件各自 50ms 计闸 —— 确认槽与 tool/result 同槽,槽位去重会吞掉快速连点面板时的重开确认音;
...
- **锚点**:触发器 = `aria-haspopup="menu"` 且祖先含 `[data-composer-seat]`(座席容器;注意 `data-composer-card` 只圈输入卡片,权限/工作区座席在卡片外 hero 行);条目 = `role="menuitem"` / `role="menuitemradio"` / `role="option"`。类名是 CSS module 运行时哈希,不作锚点。
...
- **键盘同权**:方向键移动焦点经 focus 事件响菜单移动音,与悬停同一策略。
...
6. §5.1 交互音细则逐条过:悬停逐格连响不叠加、面板打开 150ms 内抑制、未选中关闭取消音、键盘焦点移动同权、composer 卡片外菜单不响;dsh 面板 aria 结构变更时插件静默不报错。
