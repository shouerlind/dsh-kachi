...
**做**:web 界面操作确认音、agent 状态提示音、后台重要通知、设置页(含全包选音)。

**不做**:
- 公开分发 / 上架 dsh-market(版权音效仅限个人自用;仓库已私有);
- 非 web 界面的音效;
- 全局 UI 点击事件总线(dsh 无此机制,只覆盖插件自家注入组件);
...
| C:本地 | connection 流、control 流 `jobs` 帧、自家组件 DOM | `reconnecting`/`onConnected`、后台作业 `completed`/`failed`、注入组件点击 |
...
- **节流**:同类音效 200ms 时间窗内去重(初值,可在设置中调整);适用于 button/confirm 等高频槽位;
...
| 8 | `turn/end` aborted 且用户手动取消(B) | 前台 | 取消音 | `cancel.wav`(SeFooterDecideBack) | 100% |
| 9 | `tool/call`(B) | 前台 | 按键音 | `button.wav`(SeBtnDecide) | 30% |
| 10 | `tool/result` 成功(B) | 前台 | 确认音 | `confirm.wav`(SeToggleBtnOn) | 60% |
