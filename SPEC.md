...
| 25 | 全站按钮点击(泛化;面板内条目/触发器/座席按钮/自家设置行除外) | 前台 | 按键音 | `button.wav`(SeBtnDecide) | 30% |
| 26 | 全站按钮悬停 / 键盘焦点移动(同上排除) | 前台 | 菜单移动音 | `menu-move.wav`(SeBtnFocus) | 40% |
...
- **全站按钮泛化(2026-09-08)**:锚点 = `button,[role="button"]`;排除集:面板容器子树(条目路径权威,面板内非条目区域静默)、自家设置行 `.kachi-row`(own-click 已接线 §5 行 18)、座席按钮(差分兜底权威;触发器/条目在 click 分流中先于泛化命中)。点击=按键音(`ui-click`);悬停与键盘焦点=菜单移动音(`ui-hover`),共用「最近发声目标」去重(同一按钮不重复响),离开按钮面(null)重进可再响;按压伴随 focus 静默(#28 同构,防点击双响)。面板在场时按到外部按钮:取消音照旧,当次 `ui-click` 抑制 —— 一次按压一声,下一次按压恢复。悬停连响由 menuMove 槽单声道打断 + 50ms 间隔天然节流,无需新机制。
...
7. 全站按钮泛化逐条过(2026-09-08):侧边栏/设置一级导航/顶栏等任意可点按钮点击=按键音、悬停与键盘焦点=菜单移动音(共用去重,同一按钮不重复);面板条目、触发器、座席按钮、自家设置行各循原路径不双响;面板在场点外按钮仅取消音,当次点击静默。
