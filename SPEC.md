...
| 21 | 菜单触发器点击·打开面板(全站:权限设置/模型选择/推理等级/设置页菜单行等) | 前台 | 确认音 | `confirm.wav`(SeToggleBtnOn) | 80% |
...
| 25 | 全站按钮点击(泛化;含会话列表 treeitem 行;面板内条目/触发器/座席按钮/自家设置行除外) | 前台 | 按键音 | `button.wav`(SeBtnDecide) | 30% |
| 26 | 全站按钮悬停 / 键盘焦点移动(同上排除,含会话列表 treeitem 行) | 前台 | 菜单移动音 | `menu-move.wav`(SeBtnFocus) | 40% |

### 5.1 交互音细则(§2 DOM 委托决议,ADR-0001)

- **锚点(工单 #20 已全局推广)**:触发器 = `aria-haspopup="menu" | "listbox"`(全站;dsh 实测:设置页菜单行/上下文量表/语言行等为 menu,composer 命令菜单 + 钮为 listbox;dialog/tree 弹层非二级面板,不接);条目 = `role="menuitem"` / `role="menuitemradio"` / `role="option"`;面板 = `role="menu"` / `role="listbox"`(含 portal 到 body 的共享面板)。类名是 CSS module 运行时哈希,不作锚点;**挂载但不可见的面板不算在**(checkVisibility 守卫:第三方 UI 可能常驻挂载隐藏菜单,防 Escape/点外路径凭空出取消音);取消音判据为「操作时有可见面板在场」——dsh 面板均为点外即关,判据与之等价,不随点外关闭的面板会多响,实测无此形态;差分兜底的存活读数同为全站可见面板。座席内**无** `aria-haspopup` 标记的按钮(命令胶囊等;dsh 全站唯一已知的无标记触发器群)走**菜单差分兜底**:点击后等渲染落定(判定窗 50ms)按面板有无差分分类——面板挂载=开(确认音)、卸载=关(取消音)、都在=换面板开(确认音)、都没有=静默;按压落在这类按钮上不触发「点外关闭」取消音(其声音由差分路径负责,防双响)。兜底仍限 composer 座席,座席外出现静默菜单时再议。
- **悬停音**:每选项进入响一次(换行才算);50ms 最小间隔 + 单声道打断;面板打开后 150ms 内抑制首次悬停音,防与确认音叠。触发器悬停音已随按钮泛化接入(2026-09-08 决议,原「触发器本身无悬停音」废止)。
- **全站按钮泛化(2026-09-08)**:锚点 = `button,[role="button"],[role="treeitem"]` —— `treeitem` 覆盖会话列表的常规行/分组行(dsh ui-workspace Rows.tsx:div 树条目 + onClick;搜索结果行是 button[role=treeitem]),行内嵌套动作按钮取最内命中不双响;排除集:面板容器子树(条目路径权威,面板内非条目区域静默)、自家设置行 `.kachi-row`(own-click 已接线 §5 行 18)、座席按钮(差分兜底权威;触发器/条目在 click 分流中先于泛化命中)。点击=按键音(`ui-click`);悬停与键盘焦点=菜单移动音(`ui-hover`),共用「最近发声目标」去重(同一目标不重复响),离开按钮面(null)重进可再响;按压伴随 focus 静默(#28 同构,防点击双响)。面板在场时按到外部按钮(含会话行):取消音照旧,当次 `ui-click` 抑制 —— 一次按压一声,下一次按压恢复。悬停连响由 menuMove 槽单声道打断 + 50ms 间隔天然节流,无需新机制。座席豁免仅限点击分流(防差分兜底双响):悬停/键盘焦点不豁免座席,send 等座席按钮悬停发声属预期。泛化路径不设 150ms 打开抑制窗:dsh 面板均为 portal 覆盖层,不位移布局,指针不会被动命中新出现的按钮,该窗只对面板条目有意义。
