...
function createEngine(options) {
  ...
  async function ensureBuffer(slotFile) {
    ...
    try {
      ...
      const envelope = await data.json();
      if (typeof envelope?.b64 !== "string") return void 0;
      const buffer = await ctx.decodeAudioData(decodeBase64(envelope.b64).buffer);
      if (closed) return void 0;
